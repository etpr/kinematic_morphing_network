...
def create(c):
    ...
    x = Conv2D(n_conv1, (3, 3), activation='tanh', padding='same', input_shape=(n_h, n_w, 1), name='conv1')(Im)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv2, (3, 3), activation='tanh', padding='same', input_shape=(n_h / 2, n_w / 2, n_conv1),
               name='conv2')(x)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv3, (3, 3), activation='tanh', padding='same', input_shape=(n_h / 4, n_w / 4, n_conv2),
               name='conv3')(x)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv4, (3, 3), activation='tanh', padding='same', input_shape=(n_h / 8, n_w / 8, n_conv3),
               name='conv4')(x)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv5, (3, 3), activation='tanh', padding='same', input_shape=(n_h / 16, n_w / 16, n_conv4),
               name='conv5')(x)
    x = MaxPooling2D()(x)
