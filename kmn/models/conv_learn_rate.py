...
def create(c):
    ...
    x = Conv2D(n_conv1, (3, 3), activation='relu', padding='same', input_shape=(n_h, n_w, 1), name='conv1',
               kernel_regularizer=kern_reg, bias_regularizer=bias_reg)(Im)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv2, (3, 3), activation='relu', padding='same', input_shape=(n_h / 2, n_w / 2, n_conv1),
               name='conv2', kernel_regularizer=kern_reg, bias_regularizer=bias_reg)(x)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv3, (3, 3), activation='relu', padding='same', input_shape=(n_h / 4, n_w / 4, n_conv2),
               name='conv3', kernel_regularizer=kern_reg, bias_regularizer=bias_reg)(x)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv4, (3, 3), activation='relu', padding='same', input_shape=(n_h / 8, n_w / 8, n_conv3),
               name='conv4', kernel_regularizer=kern_reg, bias_regularizer=bias_reg)(x)
    x = MaxPooling2D()(x)
    x = Conv2D(n_conv5, (3, 3), activation='relu', padding='same', input_shape=(n_h / 16, n_w / 16, n_conv4),
               name='conv5', kernel_regularizer=kern_reg, bias_regularizer=bias_reg)(x)
    x = MaxPooling2D()(x)
    ...
    model.compile(loss='mean_squared_error', metrics=['mean_absolute_error'], optimizer=Adam(lr=0.001))
